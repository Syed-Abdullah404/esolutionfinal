                  ...
                  <div class="row">
                     <div class="col-lg-12">
                        <table id="example" class="table table-striped table-responsive mx-10">
                           ...
                           <?php
                                         $idt=$_GET['idtlb'];
                                        $quiz_sel=mysqli_query($conn,"select * from teacher_xtra_lab where stu_id='$idt'");
...
<?php 
   ...
   if(isset($_POST['addFile'])){
    ...
    if($file_error == 0){
     ...
     $quiz_id=$_GET['lab'];
     $file_id=$_GET['fid'];
    $sql=mysqli_query($conn,"INSERT INTO `teacher_xtra_lab`(`x_id`,`lab_id`,`stu_id`,`teacher_id`, `name`, `description`, `stu_file`, `course`) VALUES ('$file_id','$quiz_id','$idt','$teacher','$name','$description','$destinaton','$courseStudent')");
    
    if($sql){
       		$to_email="check9404@gmail.com";;
      		$t_email =$_SESSION['teacher'];
            $subject="Lab Task";
            $body=" Hi Admin ,  $t_email added a lab task ..Check and response the request";
            $header="From: $t_email";
      ...
      ?>
         <script>
           window.location="<?php echo $app_url .'teacher/lab.php?id='."$teacher" ?>";
         </script>
      <?php
    }
  }
