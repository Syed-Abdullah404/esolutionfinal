                  ...
                  <div class="row">
                     <div class="col-lg-12">
                        <table id="example" class="table table-striped table-responsive mx-10">
                           ...
                           <?php
                                         $idt=$_GET['idtp'];
                                        $quiz_sel=mysqli_query($conn,"select * from teacher_xtra_project where stu_id='$idt'");
                                        if($quiz_sel){
           ...
           // alert(deleteid);
            swal({
                ...
                if (willDelete) {
                   $.ajax({
                       type: "POST",
                       url: "teacher_xtra_project_del.php",
                       data: {
                           "delete_btn_set":1,
                           "deleteid":deleteid,
                       },                      
...
<?php 
   ...
   if(isset($_POST['addFile'])){
    ...
    if($file_error == 0){
     $destinaton='TeacherProjectFile/'.$file_name;
     move_uploaded_file($file_path,$destinaton);
     $idt=$_GET['idtp'];
     $courseStudent=$_POST['course'];
     $quiz_id=$_GET['project'];
     $file_id=$_GET['fid'];
     $idteacher=$_GET['id'];
    $sql=mysqli_query($conn,"INSERT INTO `teacher_xtra_project`(`x_id`,`project_id`,`stu_id`,`teacher_id`, `name`, `description`, `stu_file`, `course`) VALUES ('$file_id','$quiz_id','$idt','$idteacher','$name','$description','$destinaton','$courseStudent')");
    
    if($sql){
        $idtea=$_GET['id'];
     mysqli_query($conn,"INSERT INTO `admin_teacher_pro_request`(`x_id`,`project_id`,`name`, `description`, `file`, `course_name`, `stu_id`, `teacher_id`, `activity`) VALUES ('$file_id','$quiz_id','$name','$description','$destinaton','$courseStudent','$idt','$idtea','quiz')");
      	
      		$to_email="check9404@gmail.com";
      		$t_email =$_SESSION['teacher'];
            $subject="Lab Task";
            $body=" Hi Admin ,  $t_email added a project task ..Check and response the request";
            $header="From: $t_email";
      ...
      ?>
         <script>
           window.location="<?php echo $app_url .'teacher/project.php?id='."$idteacher" ?>";
         </script>
      <?php
    }
  }
