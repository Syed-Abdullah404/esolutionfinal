                  ...
                  <div class="row">
                     <div class="col-lg-12">
                        <table id="example" class="table table-striped table-responsive mx-10">
                           ...
                           <?php
                                         $idt=$_GET['idtass'];
                                        $quiz_sel=mysqli_query($conn,"select * from teacher_xtra_assignment where stu_id='$idt'");
                        ...
                        <div class="form-group" >
                           ...
                           <div class="form-group">
                               <?php
                                     ...
                                     ?>
                              <input type="hidden" name="teacher" value="<?php echo $row['id']; ?>">
                                     <label class="fw-bold">Course:</label>
                                     <select name="course" class="form-control">
                                     <?php
                                       
                              
                                            $course=mysqli_query($conn,"select * from admin_teacher_course where teacher_name='$course_name'");
                                            while($course_row=mysqli_fetch_assoc($course)){
                                                    $course=$course_row['id'];
                                           
                                                $get_course=mysqli_query($conn,"select * from admin_course where id='$course'");
...
<?php 
   ...
   if(isset($_POST['addFile'])){
    ...
    if($file_error == 0){
     $destinaton='TeacherAssignemntFile/'.$file_name;
     move_uploaded_file($file_path,$destinaton);
     $idt=$_GET['idtass'];
     $courseStudent=$_POST['course'];
     $teacher=$_POST['teacher'];
     $quiz_id=$_GET['assignment'];
     $file_id=$_GET['fid'];
    $sql=mysqli_query($conn,"INSERT INTO `teacher_xtra_assignment`(`x_id`,`ass_id`,`stu_id`, `teacher_id`, `name`, `description`, `stu_file`, `course`) VALUES ('$file_id','$quiz_id','$idt','$teacher','$name','$description','$destinaton','$courseStudent')");
    
    if($sql){
      	// $to_email = "fix404error@gmail.com";
      	// 	$t_email =$_SESSION['teacher'];
      	// 	$to_email="check9404@gmail.com";
         //    $subject="Teacher Lab Task";
         //    $body="Hi  Admin,  $t_email added a assignment task . Check and response the request";
         //    $header="From: $t_email";
      ...
      ?>
         <script>
           window.location="<?php echo $app_url .'teacher/assignment.php?id='."$teacher" ?>";
         </script>
      <?php
    }
  }
