...
include '../connection.php';
   ...
   define("TITLE","Student Finals Request");
   define("PAGE","Student Finals Request");
   ...
   <div class="container ">
      <div class="card ">
         <div class="card-header border-0">
            <h3 class="card-title display-6">Student Finals Request</h3>
            <hr>
            ...
            <tbody>
               ...
               if ($query) {
                  while ($row = mysqli_fetch_assoc($query)) {
                     ...
                     $stu_id = $row['id'];
                     $st_id = $row['stu_id'];
                     $sel_course = mysqli_query($conn, "select * from student_xtra_final where id='$course_id'");
...
<?php 
   if(isset($_POST['save']))
   {
     ...
     $sql=mysqli_query($conn,"INSERT INTO `teacher_finals`(`x_id`,`final_id`, `stu_id`,`teacher_id`, `name`, `description`, `stu_file`, `course`, `start_time`, `end_time`, `start_date`, `end_date`,`date`) VALUES ('$xid','$qtid','$stid','$teacher','$name','$description','$file','$course','$stime','$etime','$sdate','$edate',NOW())");
     if($sql){
       mysqli_query($conn,"UPDATE `student_xtra_final` SET `status`='apporve' WHERE final_id='$qtid' AND id='$xid'");
       ...
       $temail=mysqli_query($conn,"SELECT * FROM `admin_teacher` WHERE id='$teacher'");
       		...
       		$to_email =$trow['email'];
      		$t_email ='';
            $subject="Assigning Of Assignment Task";
         ...
         ?>
<script>
   window.location="<?php echo $app_url .'admin/finalStudentTab.php' ?>";
</script>
<?php
   }
   }else{
   ...
   if(isset($_POST['reject'])){
      ...
      $xid=$_POST['id'];
     $sttid=$_POST['sttid'];
     $update= mysqli_query($conn,"UPDATE `student_xtra_final` SET `status`='reject' WHERE final_id='$qtid' AND id='$xid'");
     if($update){
        $sremail=mysqli_query($conn,"SELECT * FROM `admin_student` WHERE id='$sttid'");
            ...
            $body=" Hi Student , Admin Reject your Assignment Request Check your portal..";
            $header="From: check9404@gmail.com";
...
mysqli_query($conn, "UPDATE `student_xtra_final` SET `fee`='$fee_student' WHERE final_id='$assignment_id' AND id='$id'");
