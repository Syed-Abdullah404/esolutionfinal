...
<?php 
   if(isset($_POST['save']))
   {
     ...
     $sql=mysqli_query($conn,"INSERT INTO `teacher_lab`(`x_id`,`lab_id`, `stu_id`,`teacher_id`, `name`, `description`, `stu_file`, `course`, `start_time`, `end_time`, `start_date`, `end_date`, `date`) VALUES ('$xid','$qtid','$stid','$teacher','$name','$description','$file','$course','$stime','$etime','$sdate','$edate',NOW())");
     if($sql){
            ...
            $temail=mysqli_query($conn,"SELECT * FROM `admin_teacher` WHERE id='$teacher'");
       		...
       		$to_email =$trow['email'];
            $subject=" Assigning Of lab Task";
            $body=" Hi Teacher, admin  assign a lab task to you Check your Portal ..";
            ...
            if (mail($to_email, $subject, $body, $header)) {
       		...
       		$to_email =$srow['email'];
            $subject="Approval Of Lab Task";
            $body=" Hi Student , Admin Approve your Lab activity Request..";
            $header="From: check9404@gmail.com";
       ...
       mysqli_query($conn,"UPDATE `student_xtra_lab` SET `status`='apporve' WHERE lab_id='$qtid' AND id='$xid'");
         ?>
<script>
   window.location="<?php echo $app_url .'admin/labStudentTab.php' ?>";
</script>
<?php
   }
}else{
   ...
   if(isset($_POST['reject'])){
      ...
      $xid=$_POST['id'];
      $sttid=$_POST['sttid'];
     $update= mysqli_query($conn,"UPDATE `student_xtra_lab` SET `status`='reject' WHERE lab_id='$qtid' AND id='$xid'");
     if($update){
            ...
            $subject="Rejection Of Lab Task";
            $body=" Hi Student , Admin Reject your Lab Request Check Your Portal..";
            $header="From: check9404@gmail.com";
