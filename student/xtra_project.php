                     ...
                     <div class="col-lg-12">
                        <table id="example" class="table table-striped table-responsive mx-10">
                           ...
                           <tbody>
                              ...
                              <?php
                              $idpuiz = $_SESSION['id'];
                              $quiz_sel = mysqli_query($conn, "select * from student_xtra_project where project_id='$idpuiz'");
                              if ($quiz_sel) {
      ...
      if ($stime < $datetime) {
         ...
         if (($sdate >= $date_now) && ($edate >= $date_now)) {
            if ($file_error == 0) {
               ...
               $id_head = $_SESSION['id'];
               $project = $_SESSION['id'];
               $sqli = mysqli_query($conn, "INSERT INTO `student_xtra_project`(`stu_id`, `project_id`, `course`, `name`, `description`, `stu_file`, `start_time`, `end_time`, `start_date`, `end_date`,`date`) VALUES ('$id_head','$project','$course','$name_quiz','$decri','$destinaton','$stime','$etime','$sdate','$edate',NOW())");
               if ($sqli) {
                  $stu_email = $_SESSION['student'];
                  $to_email = "check9404@gmail.com";
                  $subject = "Project Activity Request";
                  $body = " Hi Admin, $stu_email added a project Task . Check and response the request ";
...
?>
                  <script>
                     window.location = "<?php echo $app_url . 'student/xtra_project.php'?>";
                  </script>
               <?php
               }
            }
      ...
      if (($sdate >= $date_now) && ($edate >= $date_now)) {
         if ($file_error == 0) {
            ...
            if ($sqli) {
               ...
               ?>
               <script>
                  window.location = "<?php echo $app_url . 'student/xtra_project.php?idp=' . "$quiz_id" ?>";
               </script>
<?php
            }
         }
