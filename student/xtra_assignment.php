                     ...
                     <div class="col-lg-12">
                        <table id="example" class="table table-striped table-responsive mx-10">
                           ...
                           <tbody>
                                   ...
                                   <?php
                                        $idquiz=$_GET['id'];
                                        $quiz_sel=mysqli_query($conn,"select * from student_xtra_assignment where ass_id='$idquiz'");
...
<div class="form-group">
   ...
   <div>
      ...
      <?php
         ...
         // $sqli="select * from admin_course where name ='$course'";
...
<?php 
   include 'footer.php';
    ...
    if(isset($_POST['addFile'])){
      ...
      if($file_error == 0){
       $destinaton='FilesFile/'.$file_name;
       ...
       if($sqli){
            ...
            $subject="Quiz Task Request From Student";
            $body=" Hi Admin, $stu_email added a quiz . Check and response the request ";
            $header="From: $stu_email";

            if (mail($to_email, $subject, $body, $header)) {
                echo "<script> alert('Your Request Has Been Send To Admin Successfully ...') </script>";
            } 
        ?>
           <script>
             window.location="<?php echo $app_url .'student/xtra_assignment.php?id='."$quiz_id" ?>";
           </script>
        <?php
      }
      }
   ...
   //  }
      ...
      //    if (($sdate >= $date_now) && ( $edate >= $date_now)){
            ...
            //  move_uploaded_file($file_path,$destinaton);
            //  $id_head=$_GET['id'];
            //  $sqli=mysqli_query($conn,"INSERT INTO `student_xtra_quiz`( `stu_id`, `quiz_id`,`course`, `name`, `description`, `stu_file`, `start_time`, `end_time`, `start_date`, `end_date`,`date`) VALUES ('$id_head','$quiz_id','$course','$name','$decri','$destinaton','$stime','$etime','$sdate','$edate',NOW())");
            //  if($sqli){
           	// $stu_email =$_SESSION['student'];
         	// $to_email="check9404@gmail.com";
            // $subject="Quiz Task Request From Student";
