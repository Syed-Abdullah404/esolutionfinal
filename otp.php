...
$getid = $_GET['id'];
$sqli = "select * from student_sign_up where id = '$getid'";
$sql = mysqli_query($conn, $sqli);
while ($row = mysqli_fetch_assoc($sql)) {
    $checkotp = $row['otp_code'];
}
...
if($otp == $checkotp){
     ...
     $check = "UPDATE `student_sign_up` SET `approval`='YESLOG' WHERE id =$getid ";
     if($check){
        $sqli = "select * from student_sign_up where id = '$getid'";
        $sql = mysqli_query($conn, $sqli);
        while ($row = mysqli_fetch_assoc($sql)) {
        ...
        $approve_login = mysqli_query($conn, "INSERT INTO `admin_student`(`name`, `cnic`, `faculty`, `mobile`, `email`, `password`, `cpassword`, `address`, `session`, `start_date`, `end_date`, `status`) VALUES ('$name','0','0','$mobile','$email','$password','$cpassword','','0',NOW(),NOW(),'pending')");
        } 
    }
