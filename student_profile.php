...
// $sql = mysqli_query($conn, "select * from student_sign_up where id='$ids'");
    ...
    <div class="row shadow">
      ...
      <div class="col-lg-3 col-sm-12  text-end">
        <button class="btn " type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
          <img src="./student/images/dummy-dp.png" style="border-radius: 100px;" class="m-4" alt="" height="50px" width="60px">
        </button>
...
//   include 'footer.php';
  ...
  if (isset($_POST['submit'])) {
    ...
    if($faculty == ''){
      echo "<script>alert('Please fill the faculty portion !! ') </script>";
    }
    ...
    if($sql){
        ...
        foreach ($course as $cour) {
            $select_student = "select * from admin_student where id=". $ids;
            $run = mysqli_query($conn, $select_student);
            while ($row = mysqli_fetch_assoc($run)) {
                $id = $row['id'];
            }
            $student = mysqli_query($conn, "INSERT INTO `admin_student_course`(`student_id`, `course`) VALUES ('$id','$cour') ");
        }
    }  
